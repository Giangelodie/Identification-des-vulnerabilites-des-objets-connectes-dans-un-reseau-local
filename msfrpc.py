...
class MsfRpcClient(object):
    ...
    def __init__(self, password, **kwargs):
        ...
        if self.token is None:
            ...
            self.login(self.token, password)
...
class MeterpreterSession(MsfSession):
    ...
    def load_plugin(self, plugin):
        ...
        out = self.run_with_output('load {plugin}', end_strs)
    ...
    def gather_output(self, cmd, out, end_strs, timeout, timeout_exception):
        ...
        if timeout_exception:
            msg = "Command <{repr(cmd)[1:-1]}> timed out in <{timeout}s> on session <{self.sid}>"
            if end_strs == None:
                msg += " without finding any termination strings within <{end_strs}> in the output: <{out}>"
            raise MsfError(msg)
        else:
            return out
    ...
    def import_psh(self, script_path):
        ...
        end_strs = ['[-]', '[+]']
        out = self.run_with_output('powershell_import {script_path}', end_strs)
        if 'failed to load' in out:
            raise MsfRpcError('File {script_path} failed to load.')
        return out
    ...
    def run_psh_cmd(self, ps_cmd, timeout=310, timeout_exception=True):
        ...
        if 'powershell' not in self.info['plugins']:
            self.load_plugin('powershell')
        ps_cmd = 'powershell_execute "{ps_cmd}"'
...
class ShellSession(MsfSession):
    ...
    def gather_output(self, cmd, end_strs, timeout):
        ...
        raise MsfError("Command <{repr(cmd)[1:-1]}> timed out in <{timeout}s> on session <{self.sid}> "
                       "without finding any termination strings within <{end_strs}> in the output: <{out}>")
